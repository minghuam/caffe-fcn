...
HandObjectDataLayer<Dtype>::~HandObjectDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void HandObjectDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  HandObjectDataParameter data_param = this->layer_param_.hand_object_data_param();
  ...
  string ho_image_path;

  LOG(INFO) << "Opening file " << source;
  std::ifstream in_file(source.c_str());
  while(in_file >> image_path >> gt_image_path >> ho_image_path){
    ImagePair pair;
    pair.image = image_path;
    pair.gt_image = gt_image_path;
    pair.ho_image = ho_image_path;
...
template <typename Dtype>
void HandObjectDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
bool HandObjectDataLayer<Dtype>::RandBool(){
  caffe::rng_t* rng =
...
template <typename Dtype>
void HandObjectDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for(int item_id = 0; item_id < batch_size; item_id++){
    ...
    cv::Mat I_ho = cv::imread(image_pairs_[image_pair_id_].ho_image); 
    for(int c = 0; c < 2; ++c){
        for(int h = 0; h < I_ho.rows; ++h){
            for(int w = 0; w < I_ho.cols; ++w){
                int x = w;
                int y = h;
                if(c == 0){
                    // object
                    float label = I_ho.at<cv::Vec3b>(y, x)[c] > 0 ? 1 : 0;
                    datum.add_float_data(label);
                }else{
                    // hands
                    int left = I_ho.at<cv::Vec3b>(y, x)[c];
                    int right = I_ho.at<cv::Vec3b>(y, x)[c+1];
                    int label = 0;
                    if(left > 0 || right > 0){
                        if(left >= right){
                            label = 1;
    ...
    if(show_level == 1){
      cv::addWeighted(I_image, 0.85, I_ho, 0.5, 0, I_image);
      cv::imshow("I", I_image);
      cv::waitKey(30);
    }
...
INSTANTIATE_CLASS(HandObjectDataLayer);
REGISTER_LAYER_CLASS(HandObjectData);
