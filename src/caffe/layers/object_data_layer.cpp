...
ObjectDataLayer<Dtype>::~ObjectDataLayer<Dtype>() {
  this->JoinPrefetchThread();
}

template <typename Dtype>
void ObjectDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  ObjectDataParameter data_param = this->layer_param_.object_data_param();
  ...
  string object_image_path;

  LOG(INFO) << "Opening file " << source;
  std::ifstream in_file(source.c_str());
  while(in_file >> image_path >> gt_image_path >> object_image_path){
    ImagePair pair;
    pair.image = image_path;
    pair.gt_image = gt_image_path;
    pair.object_image = object_image_path;
...
template <typename Dtype>
void ObjectDataLayer<Dtype>::ShuffleImages() {
...
template <typename Dtype>
bool ObjectDataLayer<Dtype>::RandBool(){
  caffe::rng_t* rng =
...
template <typename Dtype>
void ObjectDataLayer<Dtype>::InternalThreadEntry() {
  ...
  for(int item_id = 0; item_id < batch_size; item_id++){
    ...
    cv::Mat I_object = cv::imread(image_pairs_[image_pair_id_].object_image); 
    for(int h = 0; h < I_object.rows; ++h){
      for(int w = 0; w < I_object.cols; ++w){
        int x = w;
        int y = h;
        if(do_mirror){
          x = I_object.cols - 1 - x;
        }
        float label = I_object.at<cv::Vec3b>(y, x)[0] > 0 ? 1 : 0;
        datum.add_float_data(label);
      }
    }
    ...
    if(show_level == 1){
      for(int h = 0; h < I_object.rows; ++h){
        for(int w = 0; w < I_object.cols; ++w){
          I_object.at<cv::Vec3b>(h,w)[2] = I_label.at<uchar>(h,w);
        }
      }
      
      cv::addWeighted(I_image, 0.85, I_object, 0.5, 0, I_image);
      cv::imshow("I", I_image);
      cv::waitKey(30);
    }
...
INSTANTIATE_CLASS(ObjectDataLayer);
REGISTER_LAYER_CLASS(ObjectData);
